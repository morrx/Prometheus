...
Color damagecolors = Color(int(g_Options.Colors.damageindicator[0] * 255.f), int(g_Options.Colors.damageindicator[1] * 255.f), int(g_Options.Colors.damageindicator[2] * 255.f));
...
void DamageIndicators::paint() {
	...
	for (int i = 0; i < data.size(); i++) {
		...
		if (g_Render->WorldToScreen(data[i].Position, screen_pos)) {
			g_Render->Textf(screen_pos.x, screen_pos.y, damagecolors, g_Render->font.ESP, std::to_string(data[i].iDamage).c_str());

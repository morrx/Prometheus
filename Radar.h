...
void DrawRadar()
{
		...
		if (ImGui::Begin(("Radar"), &g_Options.Misc.radarwindow, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar ))
		{
			...
			if (g_Engine->IsInGame() && g_Engine->IsConnected())
			{
				...
				for (int i = 0; i < g_Engine->GetMaxClients(); i++) {
					...
					Vector EntityPos = misc::RotatePoint(pBaseEntity->GetOrigin(), LocalPos, pos.x, pos.y, siz.x, siz.y, ang.y, g_Options.Misc.radrzoom, &viewCheck);

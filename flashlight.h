...
{
	static DWORD oConstructor = U::FindSig("client.dll", "55 8B EC F3 0F 10 45 ? B8");
...
{
	static DWORD oDestructor = U::FindSig("client.dll", "56 8B F1 E8 ? ? ? ? 8B 4E 28");
...
void UpdateFlashLight(CFlashLightEffect *pFlashLight, const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp)
{
	...
	if (!oUpdateLight)
	{
		...
		DWORD callInstruction = U::FindSig("client.dll", "E8 ? ? ? ? 8B 06 F3 0F 10 46"); // get the instruction address
...
void FlashlightRun(C_BaseEntity *pLocal)
{
	...
	if (GetAsyncKeyState(flashlightkey))
	{
		if (!pFlashLight)
		{
				pFlashLight = CreateFlashLight(pLocal->GetIndex(), "effects/flashlight001", 35, 1000, 1000);
		}
		else
		{
